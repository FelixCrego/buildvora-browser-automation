  ...
  return (
    <Panel title="Accounts" kicker="Tenant controls">
      <div className="overflow-hidden rounded-[1.5rem] border border-slate-200">
        <div className="grid grid-cols-[1.2fr_0.9fr_0.8fr_0.8fr_0.7fr_0.8fr_0.8fr] gap-4 bg-[#f5f5f7] px-5 py-3 text-[10px] uppercase tracking-[0.18em] text-slate-500">
          <span>Account</span>
          <span>Plan</span>
          <span>Status</span>
          <span>Credits</span>
        ...
        {accounts.map((account) => (
          <div key={account.id} className="grid grid-cols-[1.2fr_0.9fr_0.8fr_0.8fr_0.7fr_0.8fr_0.8fr] gap-4 border-t border-slate-200 bg-white px-5 py-4 text-sm text-slate-600">
            <div>
              <p className="font-semibold text-slate-950">{account.name}</p>
              <p className="mt-1 text-slate-500">{account.vertical}</p>
            </div>
            <div>{account.planName}</div>
            ...
            <div>
              ...
              <p className="mt-1 text-slate-500">soft limit {account.softLimitCredits}</p>
            </div>
